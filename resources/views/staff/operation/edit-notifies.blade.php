  ...
  <div class="page-header">
    <div class="page-header-content">
      <div class="page-title">
        <h2>
          <a href="{{ route('Staff::operation@notifies') }}" class="btn btn-link">
          ...
          <div class="col-md-offset-3 col-md-6">
            ...
            <div class="panel panel-flat">
              <div class="panel-body">
                  <form method="POST" enctype="multipart/form-data" action="http://bus.runtime.mobi/staff@kimochi/operation/{{$notifies->id}}/editNotifies">
                  {{ csrf_field() }}
                  <div class="form-group">
                    <label for="name" class="control-label text-semibold">Tên Notification</label>
                    <i class="icon-question4 text-muted text-size-mini cursor-pointer js-help-icon" data-content="Tên Notifier"></i>
                    <input type="text" id="name" name="name" class="form-control" value="{{$notifies->name}}" />
                    @if ($errors->has('name'))
                  ...
                  <div class="form-group ">
                    ...
                    <textarea id="contact-info" name="content" rows="4" cols="4" class="form-control">{{$notifies->content}}</textarea>
                  ...
                  <div class="form-group ">
                    ...
                    <div class="media no-margin-top">
                      <div class="media-left">
                        <img src="{{$notifies->image_video ? \App\Models\Enterprise\ProductDone::get_image_url($notifies->image_video) : 'http://bus.runtime.mobi/assets/images/image.png'}}" style="width: 64px; height: 64px;" alt="">
                      </div>
                  ...
                  <div class="row">
                    <div class="col-md-4">
                        <select class="form-control" id="status-filter" name="obj">
                          ...
                          @foreach (\App\Models\Enterprise\Notification::$statusObj as $k => $v)
                            <option value="{{ $k }}" {{$notifies->obj == $k ? "selected" : ""}}>{{ $v }}</option>
                          @endforeach
                      </select>
                    </div>
                    <div class="col-md-4">
                        <select class="form-control" id="status-filter" name="status">
                          ...
                          @foreach (\App\Models\Enterprise\Notification::$statusPost as $k => $v)
                            <option value="{{ $k }}" {{$notifies->status == $k ? "selected" : ""}}>{{ $v }}</option>
                          @endforeach
                      </select>
                    </div>
                    <div class="col-md-4">
                        <select class="form-control" id="status-filter" name="cate">
                          ...
                          @foreach (\App\Models\Enterprise\Notification::$objectType as $k => $v)
                            <option value="{{ $k }}" {{$notifies->cate == $k ? "selected" : ""}}>{{ $v }}</option>
                          @endforeach
                      </select>
                    </div>
                  </div>
                  ...
                  <div class="text-right clearfix" style="margin-top: 20px;">
                    <button type="submit" class="btn btn-primary">{{ isset($notifies) ? 'Cập nhật' : 'Thêm mới' }}</button>
                  </div>
                </form>
              </div>
            </div>
