                    ...
                    <div class="col-md-8 col-md-offset-2">
                        <div class="panel panel-flat">
                            <div class="panel-body">
                                <form action="{{ route('Staff::Management::settings@grouptype.update', $survey->id) }}" method="post" enctype="multipart/form-data">
                                    {!! csrf_field() !!}
                                    {!! method_field('PUT') !!}
                                    <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                                        ...
                                        <div class="media no-margin-top">
                                            <div class="media-left">
                                                <img src="{{ $survey->image ? get_image_url($survey->image, 'thumb_small') : asset('assets/images/image.png') }}" style="width: 64px; height: 64px;" alt="">
                                            </div>
                                    ...
                                    <div class="form-group {{ $errors->has('message') ? 'has-error has-feedback' : '' }}">
                                        ...
                                        <input type="text" name="message" class="form-control" value="{{ old('message') ? old('message') : $survey->message }}" />
                                    ...
                                    <div class="form-group {{ $errors->has('location') ? 'has-error has-feedback' : '' }}">
                                        ...
                                        <input type="text" name="location" class="form-control tokenfield-primary" value="{{ is_array($survey->location) ? implode(', ', $survey->location) : '' }}" />
                                    ...
                                    <div class="form-group {{ $errors->has('link') ? 'has-error has-feedback' : '' }}">
                                        ...
                                        <input type="text" name="link" class="form-control" value="{{ old('link') ? old('link') : $survey->link }}" />
                                        @if ($errors->has('link'))
                                            <div class="form-control-feedback">
                                                <i class="icon-notification2"></i>
                                            </div>
                                    ...
                                    <button type="submit" class="btn btn-default">Cập nhật survey</button>
                                </form>
                            </div>
                        </div>
                    </div>
