    ...
    <div class="page-header-content">
      <div class="page-title">
        <h2>
          ...
          {{ isset($role) ? 'Sửa group DN' : 'Thêm group DN' }}
        </h2>
      </div>
    </div>
  </div>
          ...
          <div class="col-md-offset-2 col-md-8">
            ...
            <div class="panel panel-flat">
              <div class="panel-body">
                <form method="POST" enctype="multipart/form-data" action="{{ isset($role) ? route('Staff::Management::businessPermission@role@update', [$role->id] ): route('Staff::Management::businessPermission@role@store') }}">
                  {{ csrf_field() }}
                  @if (isset($role))
                  ...
                  <div class="form-group {{ $errors->has('quota') ? 'has-error has-feedback' : '' }}">
                    <label for="name" class="control-label text-semibold">quota</label>
                    <i class="icon-question4 text-muted text-size-mini cursor-pointer js-help-icon" data-content="Tên của Doanh nghiệp"></i>
                    <input type="number" id="quota" name="quota" class="form-control" value="{{ old('quota') ?: @$role->quota }}" />
                    @if ($errors->has('quota'))
                      <div class="form-control-feedback">
                        <i class="icon-notification2"></i>
                      </div>
                      <div class="help-block">{{ $errors->first('quota') }}</div>
                    @endif
                  </div>
                    <div class="panel panel-flat">
