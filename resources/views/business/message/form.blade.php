    ...
    <div class="page-header">
        <div class="page-header-content">
            <div class="page-title">
                <h2>
                    <a href="{{ route('Business::message@index') }}" class="btn btn-link">
                        <i class="icon-arrow-left8"></i>
                    </a>
                    {{ isset($message) ? 'Sửa tin nhắn ' . $message->title : 'Tạo tin nhắn mới' }}
                </h2>
            </div>
        </div>
    </div>
                       ...
                       @include('_partials.flashmessage')
                        <div class="panel panel-flat">
                            <div class="panel-body">
                                <form method="POST" enctype="multipart/form-data" action="{{ isset($message) ? route('Business::message@update', [$message->id]) : route('Business::message@store') }}">
                                    {{ csrf_field() }}
                                    @if (isset($message))
                                    ...
                                    <div class="form-group {{ $errors->has('content') ? 'has-error has-feedback' : '' }}">
                                        ...
                                        <input type="text" id="content" name="content" maxlength="255" class="form-control"
                                               value="@if(isset($message)){{$message->content}}@else{{old('content')}}@endif"
                                    ...
                                    <div class="form-group {{ $errors->has('type_object_to') ? 'has-error has-feedback' : '' }}">
                                        ...
                                        <i class="icon-message4 text-muted text-size-mini cursor-pointer js-help-icon" data-content="Chọn loại đích đến"></i>
                                        <select name="type_object_to" class="js-select">
                                            @foreach (App\Models\Enterprise\DNMessage::$typeObjectTo as $key => $value)
                                                <option value="{{$key}}" @if(old('type_object_to') == $key) selected @endif>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group {{ $errors->has('object_to') ? 'has-error has-feedback' : '' }}">
                                        ...
                                        <input type="text" name="object_to" maxlength="255" class="form-control"
                                               value="@if(isset($message)){{$message->object_to}}@else{{old('object_to')}}@endif"
                                    ...
                                    <div class="form-group {{ $errors->has('type_send') ? 'has-error has-feedback' : '' }}">
                                        ...
                                        <i class="icon-message4 text-muted text-size-mini cursor-pointer js-help-icon" data-content="Chọn dịch vụ "></i>
                                        <select name="type_send" id="type_send" class="js-select">
                                            @foreach (App\Models\Enterprise\DNMessage::$typeSend as $key => $value)
                                                <option value="{{$key}}" @if(old('type_send') == $key) selected @endif>{{ $value }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group {{ $errors->has('object_receive') ? 'has-error has-feedback' : '' }}">
                                        ...
                                        <i class="icon-message4 text-muted text-size-mini cursor-pointer js-help-icon" data-content="Nhập đối tượng nhận"></i>
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" class="styled" value="1" @if(old('comment_product') == 1) checked @endif name="comment_product">
                                                Comment sản phẩm
                                            </label>
                                        </div>
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" class="styled" value="1" @if(old('like_product') == 1) checked @endif name="like_product">
                                                Like sản phẩm
                                            </label>
                                        </div>
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" class="styled" value="1" @if(old('scan_product') == 1) checked @endif name="scan_product">
                                                Scan sản phẩm
                                            </label>
                                        </div>
                                        <div class="radio">
                                            <label>
                                                <input type="radio" name="check_product" value="1" class="styled"  @if(old('check_product') != 2)checked @endif>
                                        ...
                                        <div class="radio">
                                            <div style="width: 30%;float:left">
                                                <label>
                                                    <input type="radio" name="check_product" value="2" class="styled" @if(old('check_product') == 2)checked @endif>
                                    ...
                                    <div class="text-right">
                                        ...
                                        <button type="submit" class="btn btn-primary">{{ isset($message) ? 'Cập nhật' : 'Gửi' }}</button>
                                    </div>
                                </form>
                            </div>
                        </div>
