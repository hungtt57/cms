...
class ActionController extends Controller
{
    public function index(Request $request)
    {
                ...
                foreach ($selectedGln as $number) {
                    ...
                    if (($handle = fopen($dataFile, 'r')) !== false) {
                        while (($row = fgetcsv($handle)) !== false) {
                            ...
                            if (in_array($row[6], array_keys($chartData))) {
                                $chartData[$row[6]][$hour->getTimestamp() * 1000] += 1;
                                $tableData[] = [
                                    'gtin' => $row[0],
                                    'product_name' => $row[1],
                                    'user_id' => $row[2],
                ...
                foreach ($selectedGln as $number) {
                    ...
                    if (($handle = fopen($dataFile, 'r')) !== false) {
                        while (($row = fgetcsv($handle)) !== false) {
                            ...
                            if (in_array($row[6], array_keys($chartData))) {
                                $chartData[$row[6]][$day->getTimestamp() * 1000] += 1;
                                $tableData[] = [
                                    'gtin' => $row[0],
                                    'product_name' => $row[1],
                                    'user_id' => $row[2],
        ...
        return view('business.analytics.action', compact('startDate', 'endDate', 'gln', 'products', 'chartData', 'tableData', 'selectedGln', 'selectedGtin'));
    }
}
